...
class Unet(nn.Module):
    def __init__(
        ...
        self.mid_cross_attn = Residual(PreNorm(mid_dim, CrossAttention(mid_dim, self.label_embedding_dim)))
            ...
            down.append(
                nn.ModuleList(
                    [
                        ...
                        Residual(PreNorm(ins, CrossAttention(ins, self.label_embedding_dim))),
            ...
            up.append(
                nn.ModuleList(
                    [
                        ...
                        Residual(PreNorm(outs, CrossAttention(outs, self.label_embedding_dim))),
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, label_emb: torch.Tensor = None) -> torch.Tensor:
        # time embedding
        ...
        for block_1, block_2, attn, cross_attn, downsample in self.down:
            ...
            if label_emb is not None:
                x = cross_attn(x, label_emb, label_emb)
        ...
        if label_emb is not None:
            x = self.mid_cross_attn(x, label_emb, label_emb)
